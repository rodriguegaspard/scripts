...
## batcheck
A very simple script that checks the battery level and status (charging or discharging) and tints the main display with ```xrandr``` when the battery level is under 20% and discharging (red tint) or at 80% and charging (green tint). I recommend creating either a cronjon or a systemd timer for it.
## volumecontrol
